...
public static void findPathsIterative(Exp1.Node root)								
	{
	    ...
	    {
		   Exp1.Node node = nodeStack.remove(nodeStack.size()-1);      				    	// These two lines of statements will create the new 'node' and the new 'path' in each iteration, the ones which are created in the previous iteration will collected by JVM garbage collector.(continue from the next line) 
		   ...
		   {
			   ArrayList<Integer> newPath = new ArrayList<>(path);					        	// it creates 'newPath' ArrayList in each iteration, here pass the path(Which we get'retriev' from the 'pathStack' in the above statement)as an argument, so the path is referencing to the current path in which we are.The ones which were created in the previous iteration, will be collected by 'JVM garbage collector'.
